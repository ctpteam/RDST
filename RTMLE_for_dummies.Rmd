...
treatment_dt <- list(
                  A=treatment_dt[,grep('^(ID|A_)',names(treatment_dt)),with=FALSE],
                  B=treatment_dt[,grep('^(ID|B_)',names(treatment_dt)),with=FALSE]
)
```
...
```
## LTMLE - Two variable for treatment, A and B. The comparison is for the target parameter "Always treat with A and never B versus
Always treat with B and never with A".
```{r echo=TRUE}
...
prepare_data(x) <- list()
protocol(x) <- list(name = "A",intervention = data.frame("A"=factor("1",levels=c("0","1"))))
protocol(x) <- list(name = "B",intervention = data.frame("B"=factor("1",levels=c("0","1"))))
target(x) <- list(name = "Risk",strategy = "additive",estimator = "tmle",protocols = c("A","B"))
x <- run_rtmle(x,time_horizon = 2)
summary(x)
