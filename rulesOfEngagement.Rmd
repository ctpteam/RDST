...
# Questions
You are welcome to contact your supervisor and collaborators. If they cannot answer your question, your supervisor or steering committee member should contact the network administrators. Statistics Denmark requests that questions are filtered through the network administrators.  Sundhedsdatastyrelsen is more problematic since they will answer questions, but also charge a high fee for answering (or not answering) any question.
...
# Servers
Our network currently has 4 operating servers numbered 4-7. Server 6 is a back-up server and available for use, but generally we advise not to use it  Servers 4, 5 & 7 are large and program updates etc. are prioritized for these servers.  Therefore only use servers 4, 5 & 7.  Note, that server 7 is a Linux server, where the SAS interface is outdated.
