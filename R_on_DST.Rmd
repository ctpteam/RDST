...
# Parquet files
Parquet files are column oriented which reduces size and makes them fast to read. This is the data format we intend to use exclusively over a period. Parquet files can be interrogated, read and written using the nanoparquet package, the arrow package or duckdb.

Parquet files comes in two flavors. The first is a plain file. this can be read with simple readers such as read_parquet from arrows.  The second is a directory of chunks where the directory has a name that ends with ".parquet".  These are highly efficient since reading can be paralellized, but they need more complex use of duckdb and arrows
...
Som important R-equivalents are shown below. Note that DUCKDB is consitently used, it is quicker than Arrow functions for producing simple summaries - for larger data manipulations is is more uncertain whether Arrow or Duckdb is quicker.
...
```
In most cases what is needed from the raw parquet files is not the full content, but a content filtered to use selected columns and further only selected rows. Two methods are recommended.  The **arrow** package is likely most efficident for simple reads of files and **duckdb** more efficient for very large datasets.  Both methods can be integreted with **dplyr** which allows intuitive R-commands to be used instead of learning **SQL** language.  The examples below uses both general methods and with dplyr.
...
With large datasets DUCKDB is anticipated to be faster than arrow, but with 
smaller datasets arrow is simpler
```{r,warning=FALSE}    
ds <- arrow::open_dataset("path to file")
ds |>
  select() |>
  filter() |>
  collect()
