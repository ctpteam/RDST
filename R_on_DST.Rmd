...
# Parquet files
Parquet files are column oriented which reduces size and makes them fast to read. This is the future raw data as of February 2026. Parquet files can be interrogated, read and written using the nanoparquet package. This package is useful for simple use where the whole datasets are interrogated:
...
colnames(arrow::open_dataset(sources = "C:/Data/parquet/mtcars") |>
  dplyr::collect()) # provides a vector of column names    
```
In most cases what is needed from the raw parquet files is not the full content, but a content filtered to use selected columns and further only selected rows.  This is effectively done using the **DUCKDB** database program.  This can be used with dplyr or using SQL directly. I will use dplyr below. 

## Read from a single parquet file
```{r,warning=FALSE,eval=FALSE}
con <- dbConnect(duckdb::duckdb(), dbdir = ":memory:") #Establish connection - in memory
...
collect() # Afslutter dannelse af dataset
...
## Read from a sequence of similar parquet files
```{r,warning=FALSE}
...
files <- list.files(  # Create the list of files - with a regular expression
 "data/patients",
  pattern = "\\.parquet$",
  full.names = TRUE
)

result <- tbl(con, read_parquet(files)) |>
