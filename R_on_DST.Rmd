...
# Parquet files
Parquet files are column oriented which reduces size and makes them fast to read. This is the future raw data as of February 2026. Parquet files can be interrogated, read and written using the nanoparquet package, the arrow package or duckdb.
...
To see various content of parqet files you can use:
```{r,warning=FALSE,eval=FALSE}
    nanoparquet::parquet_info() #shows a basic summary of the file.
...
# str:
ds |> slice_head(n = 100) |> collect() |> glimpse()
