...
A standard start up of RStudio in Statistics Denmark is very slow. One reason is the creation of the "Packages pane".  And this is slow because CRAN has been imported in total. To get a slightly quicker start you should disable this option in Global option - Packages - Management where the tick in "Enable package pane" should be removed.
...
3. Then executing the command **use_targets()** in the R console 

This sequence of actions sets up the necessary directory structure in the specified working directory. The package creates an initial script named _targets.R, which serves as the foundation for building the data analysis pipeline.
...
The provided simple example shows a very short pipeline, comprising two steps, which can be extended endlessly to accommodate complex analyses. The pipeline is executed by running **tar_make()** in the console. One of the main time saving aspects of Targets is that when running **tar_make**, any changes to the pipeline are detected and only objects that that are outdated are updated.  
...
- Organize all functions in a dedicated folder titled "functions" within the working directory and use the command **sapply(list.files("~/functions"), source)** in the _targets.R script to source all the functions pre-pipeline.
...
- **tar_visnetwork()** can be used to visualize the dependencies and status op the objects in the pipeline. 
...
# Parquet files
## What is a Parquet dataset?

Parquet is a compressed, column-oriented storage format. It is designed for analytical work.

A Parquet dataset is often a directory, not a single file:

```text
laboratorieproevesvar.parquet/
    part_000000.parquet
    part_000001.parquet
    part_000002.parquet
    ...
...
The most important is to be able to get an overview of content:
## What is in my file
...
# Open lazily - Lizily implies that only secondary information on tables is read and further that a DUCKDB # relation is made # # # with the name "con".  Other commands then refer to "con"

# Establish connection with the data - note that "con" is only used when the database engine is contacted # # directly
...
```
Define the study population first. Use that population as a filter before collecting large registry data into R.

For small and moderate datasets, `arrow` is often the simplest tool. For very large datasets, especially when joins and complex filters are needed, `duckdb` is often the safest and fastest approach.

# The usual registry workflow
...
Create pnrlist
        ↓
Use pnrlist as a filter
        ↓
Read cancer / hospital / medication / laboratory / social data
        ↓
Collect only the relevant rows and columns
        ↓
Continue analysis in data.table, dplyr or survival models
```

The `pnrlist` is the central filter used to reduce all subsequent data queries.

A minimal population filter contains one row per person:

```r
pnrlist <- population[, .(pnr)]
```
# Finding Parquet datasets

For a directory-style Parquet dataset, DuckDB usually reads the individual part files:

```r
paste0(dirstart, "/Grunddata/laboratory/laboratorieproevesvar.parquet/*.parquet")
```
...
|---|---|
| Inspect dataset structure | Arrow or DuckDB |
| Read selected columns from moderate data | Arrow |
| Simple row filters | Arrow |
| Join a huge registry file to a study population | DuckDB |
| Search large text/code variables | DuckDB |
| Aggregate before collecting | DuckDB |
| Count rows before collecting | DuckDB |
| Return a final R object for analysis | Either |

A practical rule is:

> Use Arrow to read data. Use DuckDB to search, filter, join and aggregate data before reading it into R.

# Large worked DuckDB example

This is the main pattern to copy for large datasets.

The example:

1. reads from a Parquet dataset,
2. selects only needed columns,
3. filters rows,
4. restricts to the study population,
5. optionally limits the result during testing,
6. collects the result into R,
7. changes all variable names to lower case,
8. returns a `data.table`.

The example uses laboratory data, but the pattern also applies to medication, LPR, cancer and socioeconomic data.
...
# -------------------------------------------------------------------------
# Use a small limit while developing and testing the query.
# Set TEST_LIMIT <- NULL before the final analysis.
#
# IMPORTANT:
# The limit is added at the END of the SQL query.
# Do not use head() before filtering or joining, because that tests a
# different query from the final one.
# -------------------------------------------------------------------------

TEST_LIMIT <- 10000
# TEST_LIMIT <- NULL   # Use this for the final production run


# -------------------------------------------------------------------------
# 2. Connect to DuckDB
# -------------------------------------------------------------------------
# For one large dataset, one DuckDB connection with several threads is usually
# better than many parallel R workers.
# -------------------------------------------------------------------------
...
# -------------------------------------------------------------------------
# 3. Define the Parquet dataset
# -------------------------------------------------------------------------
# Many Parquet datasets are directories containing several part files.
# The wildcard reads all part files in the dataset.
# -------------------------------------------------------------------------

labfiles <- paste0(
  dirstart,
  "/Grunddata/laboratory/laboratorieproevesvar.parquet/*.parquet"
)


# -------------------------------------------------------------------------
# 4. Upload the study population to DuckDB
# -------------------------------------------------------------------------
# pnrlist should have been created earlier in the project.
#
# Minimal example:
#   pnrlist <- population[, .(pnr)]
#
# The temporary DuckDB table is used to restrict the large registry dataset
# before data are collected into R.
# -------------------------------------------------------------------------
...
# -------------------------------------------------------------------------
# The query does four important things inside DuckDB:
#
#   a. Reads directly from Parquet.
#   b. Selects only needed columns.
#   c. Filters rows by analysis code.
#   d. Restricts to the study population.
#
# Only after this has been done is the result returned to R.
# -------------------------------------------------------------------------

limit_clause <- if (is.null(TEST_LIMIT)) {
  ""
} else {
  paste0("\nLIMIT ", as.integer(TEST_LIMIT))
}
...
sql_lab <- glue("
  SELECT
      ...
      l.samplevalue,
      l.unit
  FROM parquet_scan('{labfiles}') AS l
  INNER JOIN study_population AS p
      ON l.pnr = p.pnr
  WHERE
      l.analysiscode LIKE 'NPU03%'
...
# -------------------------------------------------------------------------
# 6. Inspect the query before running it
# -------------------------------------------------------------------------
...
# -------------------------------------------------------------------------
# 7. Count matching rows before collecting
# -------------------------------------------------------------------------
# This version deliberately does not include the development LIMIT.
# It estimates the size of the final result.
# -------------------------------------------------------------------------
...
# -------------------------------------------------------------------------
# 8. Execute and collect
# -------------------------------------------------------------------------
# Only the selected, filtered and population-restricted result is transferred
# into R.
# -------------------------------------------------------------------------

labdata <- dbGetQuery(con, sql_lab)


# -------------------------------------------------------------------------
# 9. Convert to data.table and lower-case all names
# -------------------------------------------------------------------------
# Lower-case names make later code more consistent across registers.
# -------------------------------------------------------------------------

setDT(labdata)
setnames(labdata, tolower(names(labdata)))


# -------------------------------------------------------------------------
# 10. Result
# -------------------------------------------------------------------------
# labdata is now a data.table.
# -------------------------------------------------------------------------

labdata
```

## Same example as a reusable function

For real projects, it is often cleaner to put the pattern in a function.

```r
read_lab_values <- function(
  dirstart,
  pnrlist,
  test_limit = 10000,
  threads = 20
  ...
  data.table::setnames(result, tolower(names(result)))
...
Arrow is often the simplest tool for moderate-sized datasets.
...
ds <- open_dataset(
  paste0(
    dirstart,
    "/Grunddata/cancer/tumor_aarlig.parquet"
  )
)
```

No data are read yet.
...
cancer <- ds |>
  filter(
    diagnosedato >= as.Date("2020-01-01")
  ) |>
  select(
    pnr,
    diagnosedato,
...
# Using Arrow with the population filter

For moderate-sized datasets, Arrow can be used with `semi_join()`:
...
setDT(cancer)
setnames(cancer, tolower(names(cancer)))
```

For very large datasets, prefer DuckDB.
...
```r
tbl(con, "large_dataset") |>
  head(10000) |>
  filter(...)
```

This means: take 10,000 rows first, then filter them.

That is not the same as the final query.
...
For SQL, place `LIMIT` at the end of the query.

# Counting before collecting

Before collecting millions of rows, count first.
...
This helps determine whether the result is small enough to collect.

# Using dplyr with DuckDB

`dplyr` can be used with DuckDB because `dbplyr` translates many operations to SQL.

```r
lab_query <- tbl(con, "lab") |>
  semi_join(
    tbl(con, "study_population"),
    by = "pnr"
  ) |>
  filter(
    analysiscode == "NPU03230"
  ) |>
  select(
    pnr,
    analysiscode,
...
```r
show_query(lab_query)
```

Collect only after checking the query:
...
For very large or complex queries, explicit SQL is often clearer and easier to debug.

# Parallel processing

DuckDB supports multiple threads:

```r
dbExecute(con, "SET threads=20")
```

However, not every query can use all threads. Parallelism depends on file layout, row groups, filters, joins and available I/O.

Avoid oversubscription. For example, 10 R workers each using 20 DuckDB threads can overwhelm a shared server.

General rule:

```text
Many independent datasets -> several R workers, few DuckDB threads each
One huge dataset          -> one DuckDB connection, many DuckDB threads
```
...
2. Create `pnrlist`.
3. Inspect the relevant Parquet dataset.
4. Decide whether Arrow or DuckDB is appropriate.
5. For huge datasets, upload `pnrlist` to DuckDB.
6. Select only required columns.
7. Filter rows inside DuckDB or Arrow.
8. Restrict to `pnrlist` before collecting.
9. Count before collecting.
10. Collect and convert to `data.table`.
11. Lower-case variable names.
12. Remove test limits before final analysis.

# Summary

The most efficient registry workflow is population-first.

Do not read large registry datasets and then filter in R. Instead, filter and join as close to the Parquet dataset as possible. Arrow is excellent for simple reading and moderate data. DuckDB is the preferred tool when the dataset is large and requires joins, searches or aggregation before collection into R.
...
There are additionally three extra splttingprograms **splitTwo**, **splitFromTo** and **splitSeq**.  These are wrappers to the similar lexis programs and serve the purpose of providing splitting without specifying an "event" variable.  They have been specifically included for use with LTMLE.
